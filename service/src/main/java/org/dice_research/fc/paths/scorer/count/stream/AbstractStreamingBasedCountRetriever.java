...
/**
 * This is an abstract base implementation of a count retriever. It's main
 * functionality is to send queries to a SPARQL endpoint and count the number of
 * results in the JSON result stream using the
...
public abstract class AbstractStreamingBasedCountRetriever implements AutoCloseable {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractStreamingBasedCountRetriever.class);
    ...
    protected long executeCountQuery(StringBuilder queryBuilder) {
        ...
        try {
            ...
            try (CloseableHttpResponse response = httpClient.execute(request)) {
                ...
                LOGGER.debug("Got a query result ({}) after {}ms.", result, System.currentTimeMillis() - time);
                return result;
            }
        } catch (Exception e) {
